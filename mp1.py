...
def generate_dataset_denoising(nb_samples, free_location=False, test=False):
    # Getting im_size:
    im_size = (generate_a_rectangle_noise()[0]).shape[0]
    X = np.zeros([nb_samples,im_size])
    Y = np.zeros([nb_samples,im_size])
    for i in range(nb_samples):
        noise = np.random.randint(5,45)
        if i % 10 == 0:
            print(i)
        category = np.random.randint(3)
        if category == 0:
            ...
            X[i],Y[i] = generate_a_triangle_noise(noise, free_location)[0]
    X = X / 255
